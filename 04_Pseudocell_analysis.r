...
if(length(args)!=5){
print ("Usage:")
print ("Rscript 04_Pseudocell_analysis.r sp_count.txt sp_anno.txt 20  sp_name  out_file_name_str")
...
read_tabfile <- function(fname, sep='\t'){
  ...
  out <- read.table(fname, sep=sep, header = T,check.names=F,row.names=1)
...
Pseudocell_analysis_pipeline <- function(DGE_tab_data, phenotype_tab_data, sep='\t',
  ...
  if(is.character(DGE_tab_data) && is.character(phenotype_tab_data)){
    ...
    id <- read.table(phenotype_tab_data, sep = sep, header = F,check.names=F)
  ...
  #new_good_ids <- as.matrix(new_ids_length[-short,])##edit 20210325 zrh
  #new_good_ids <- as.matrix(new_ids_length)
...
Pseudocell_analysis_pipeline(
    Args[1],
    Args[2],
    pseudocell_size = as.numeric(Args[3]),
    colname_str = Args[4],
    save_out_file = TRUE,
    out_file_name_str = Args[5])
